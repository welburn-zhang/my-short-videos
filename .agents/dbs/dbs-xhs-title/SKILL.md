...
### Step 0：dbs 前置判断（全局硬规则）

短视频封面文案、主标题、双平台标题方案都不能直接生成。先判断：
...
6. **公式不能压过真实表达。** 如果一个公式会让标题看起来更有流量，但会改变用户真正想说的意思、放大并不存在的焦虑，或让账号显得像在迎合热点，必须放弃这个公式。

### 标题质量 5 条铁律（来自全平台标题实战经验）

6. **标题留悬念，不说答案。** 用「为什么」不用「证明」，用「问题」不用「结论」。检查方法：看完标题，观众还需要点进来吗？如果标题已经把答案说完了，就是失败的标题。
7. **话题可以扩大，但不能变形。** 可以把行业术语翻译成普通人听得懂的话，但不要为了扩大流量池，把内容改成用户并不想讲的「赚钱、涨粉、不上班」等泛化卖点。
